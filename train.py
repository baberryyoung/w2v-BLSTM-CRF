...
'''
train_path = os.path.join(dir,'data','train.txt')    #args.train_path
save_path = os.path.join(dir,'model','NER')	#args.save_pat
#val_path = os.path.join(dir,'data/validation.txt')	#args.val_path
num_epochs = 40   	#args.epoch
...
X_train, y_train, X_val, y_val = helper.getTrain(train_path=train_path, val_path=None, seq_max_len=num_steps)
...
emb_dim = 128
if emb_path != None:
	embedding_matrix = helper.getEmbedding(emb_path, emb_dim)
else:
