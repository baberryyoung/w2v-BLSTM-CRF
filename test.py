...
val_path = os.path.join(dir,'data/validation.txt')	#args.val_path
num_epochs = 10	#args.epoch
emb_path = os.path.join(dir,'embedding/embedding.npy')	#args.char_emb
gpu_config = "/gpu:0"#+str(args.gpu)
model_path = os.path.join(dir,'model','入院记录NER')#args.model_path
test_path = os.path.join(dir,'data','入院记录-test.txt')
test_output_path = os.path.join(dir,'data','test_output.txt')#args.output_path
num_steps = 200#it must consist with the train
predict_path = os.path.join(dir,'data','rawdata')
start_time = time.time()

print ("preparing test data")
X_test, X_test_str, y_test = helper.getTest(test_path=test_path, seq_max_len=num_steps)
...
with tf.Session(config=config) as sess:
	with tf.device(gpu_config):
		...
		print ("testing")
		model.test(sess, X_test, X_test_str, test_output_path, y_test)
